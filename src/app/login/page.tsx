        ...
        <div className="max-w-md w-full space-y-6 sm:space-y-8 p-6 sm:p-8 bg-white rounded-2xl shadow-card-hover border border-slate-200/70">
          <div className="text-center space-y-3 sm:space-y-4">
            ...
            <div>
              ...
              <p className="mt-1 text-xs sm:text-sm text-slate-500">Kanban CRM — enter password to continue</p>
            </div>
          </div>
          <form className="mt-6 sm:mt-8 space-y-5 sm:space-y-6" onSubmit={handleSubmit}>
            {error && (
              <div className="text-xs sm:text-sm text-center text-red-600 bg-red-50 border border-red-100 rounded-lg py-2 px-3">
                {error}
              </div>
            )}
            <div className="space-y-4">
              <input
                ...
                required
                autoFocus
                className="w-full px-3.5 py-2.5 border border-slate-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-400 text-center text-base sm:text-lg placeholder:text-slate-400 transition-shadow"
              />
            </div>
            <button
              type="submit"
              disabled={loading}
              className="w-full py-2.5 px-4 bg-primary-600 text-white rounded-xl hover:bg-primary-700 active:scale-[0.98] disabled:opacity-50 font-medium transition-all text-sm sm:text-base"
            >
