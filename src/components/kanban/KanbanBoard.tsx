...
export default function KanbanBoard() {
  ...
  const [composeFromList, setComposeFromList] = useState<string[]>([]);
  ...
  // Fetch available sender mailboxes on mount
  useEffect(() => {
    fetch('/api/email-configs')
      .then((r) => (r.ok ? r.json() : []))
      .then((configs: any[]) => {
        if (Array.isArray(configs)) {
          const senders = configs.map((c) => c.smtpUser).filter(Boolean);
          const uniqueSenders = Array.from(new Set(senders));
          setComposeFromList(uniqueSenders);
          if (uniqueSenders.length > 0 && !composeFrom) {
            setComposeFrom(uniqueSenders[0]);
          }
        }
      })
      .catch(() => {});
  ...
  async function handleSendCompose(e: React.FormEvent) {
    ...
    try {
      ...
      toast.success('Outbound email sent & card created on board!');
      setComposeTo('');
      setComposeSubject('');
      setComposeBody('');
      ...
      toast.error(err?.message || 'Failed to send outbound email');
    } finally {
  ...
  function handleSelectColumn(cardIds: string[], select: boolean) {
    setSelectedIds((prev) => {
      const next = new Set(prev);
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex flex-col gap-3 mb-3 sm:mb-4">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap sm:flex-nowrap">
            {selectionMode ? (
              <>
                <button
                  onClick={() => {
                    const allIds = filteredColumns.flatMap((col) => col.cards.map((c) => c.id));
                  ...
                  className="px-2.5 py-1.5 text-xs font-semibold text-primary-700 bg-primary-50 border border-primary-200 rounded-lg hover:bg-primary-100 shadow-xs transition-all"
                >
                  {selectedIds.size >= filteredColumns.flatMap((col) => col.cards).length && filteredColumns.flatMap((col) => col.cards).length > 0
                    ? 'Deselect All'
                    : `Select All (${filteredColumns.flatMap((col) => col.cards).length})`}
                </button>
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold text-white bg-rose-600 border border-rose-600 rounded-lg hover:bg-rose-700 shadow-xs transition-all disabled:opacity-50"
                >
              ...
              <>
                <button
                  ...
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold text-white bg-primary-600 hover:bg-primary-700 rounded-lg shadow-xs transition-all"
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2.5 pt-1">
          ...
          <div className="flex items-center gap-1.5 overflow-x-auto pb-1 sm:pb-0 scrollbar-none">
            ...
            <button
              onClick={() => setGlobalSortMode((prev) => (prev === 'unreads' ? 'default' : 'unreads'))}
              className={`inline-flex items-center gap-1.5 px-3 py-1 text-xs font-semibold rounded-lg transition-all ${
                globalSortMode === 'unreads'
                  ? 'bg-indigo-600 text-white shadow-xs ring-2 ring-indigo-300'
                  : 'bg-white text-slate-700 border border-slate-200 hover:bg-slate-50'
              }`}
              title="Sort cards in each column with unread emails at the top"
            >
              <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M3 4h13M3 8h9m-9 4h6m4 0l4-4m0 0l4 4m-4-4v12" />
              </svg>
              {globalSortMode === 'unreads' ? 'Sorted: Unreads First' : 'Sort: Unreads'}
            </button>
          </div>
        </div>
      ...
      {/* Outbound Compose Message Modal */}
      {showComposeModal && (
          ...
          <div className="bg-white rounded-2xl max-w-lg w-full p-5 sm:p-6 shadow-2xl border border-slate-200 space-y-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-base font-bold text-slate-900">Compose Outbound Email</h3>
              </div>
              <button
            ...
            <p className="text-xs text-slate-500">
              Send a new outbound email directly. It will be tracked as a new pipeline card on <strong>{boardName}</strong>.
            </p>

            <form onSubmit={handleSendCompose} className="space-y-3.5 text-xs sm:text-sm">
              {/* Sender mailbox selector */}
              <div>
                <label className="block text-xs font-semibold text-slate-700 uppercase mb-1">From (Mailbox)</label>
                {composeFromList.length > 0 ? (
                  <select
                    value={composeFrom}
                    onChange={(e) => setComposeFrom(e.target.value)}
                    className="w-full px-3 py-2 border border-slate-300 rounded-xl bg-white text-xs sm:text-sm focus:ring-2 focus:ring-primary-500 focus:outline-none"
                  >
                    {composeFromList.map((addr) => (
              ...
              {/* Recipient */}
              <div>
                <label className="block text-xs font-semibold text-slate-700 uppercase mb-1">To (Client Email) *</label>
                <input
                  type="email"
                  required
                  placeholder="client@prospect.com"
                  value={composeTo}
                  onChange={(e) => setComposeTo(e.target.value)}
                  className="w-full px-3.5 py-2 border border-slate-300 rounded-xl focus:ring-2 focus:ring-primary-500 focus:outline-none text-xs sm:text-sm"
                />
              </div>
              ...
              <div>
                ...
                <input
                  type="text"
                  placeholder="e.g., Proposal & Partnership Discussion"
              ...
              {/* Body */}
              <div>
                <label className="block text-xs font-semibold text-slate-700 uppercase mb-1">Message Body *</label>
                <textarea
                  required
                  rows={5}
                  ...
                  className="w-full px-3.5 py-2.5 border border-slate-300 rounded-xl focus:ring-2 focus:ring-primary-500 focus:outline-none text-xs sm:text-sm resize-none"
                />
              </div>
              ...
              <div className="flex items-center justify-end gap-2 pt-2">
                ...
                <button
                  ...
                  className="px-5 py-2 text-xs bg-primary-600 hover:bg-primary-700 text-white font-bold rounded-xl shadow-xs disabled:opacity-50 flex items-center gap-1.5"
                >
                  <span>{sendingCompose ? 'Sending...' : 'Send Outbound Email →'}</span>
                </button>
              </div>
            </form>
          </div>
